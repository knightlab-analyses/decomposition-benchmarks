...
class PCA_niche(object):
    ...
    def niche_visual(otu,sfi,tax_index,bact_to_show,niche_plot,Pc_plot,mappingdf):
        ...
        #imputed
        observed_table_sfi=pd.DataFrame(sfi,index=otu.index.values,columns=otu.T.index.values).T #re-order to origonal otu table
        ...
        nichesetdict=dict([a, int(x)] for a, x in nichesetdict.items())
        ...
        return out_niche_linkeddf,observed_table_sfi,index_mean,index_std


    def plot_niche(out_niche_linkeddf,observed_table_sfi,mappingdf,niche_plot,Pc_plot,index_mean,index_std,le,cont=True):
        ...
        if cont==True:
            ...
            out_niche_linkeddf.T[index_mean].plot(kind='bar',width=.78, yerr=out_niche_linkeddf.T[index_std].values.T, alpha = 0.7,error_kw=dict(elinewidth=1,capsize=2,barsabove=True,ecolor='k',ms=1, mew=1),rot=0,colormap="Set3",sharey=True,fontsize=15,ax=ax1)
            ax1.legend(loc=2,prop={'size':16},bbox_to_anchor=(1.0, 1.0))
            Y=mapping[niche_plot].tolist()
            ax2.scatter(X_reduced2[:, 0], X_reduced2[:, 1], c=Y,cmap=plt.cm.cool,s=200)
            p=ax2.scatter(X_reduced2[:, 0], X_reduced2[:, 1], c=Y,cmap=plt.cm.cool,s=200)
        ...
        if cont==False:
            ...
            Y=list(map(int, Y))
            Y2=list(le.inverse_transform(Y))
            ...
            if int(len(k))>=3:
                ...
                numrows=int(len(k))
            ax2.legend(scatter_proxy, k, numpoints = 1,loc=2,prop={'size':22},bbox_to_anchor=(1.0, 1.0),fontsize = 'large', labelspacing=.1, borderaxespad=0.)
            ax2.scatter(X_reduced2[:, 0], X_reduced2[:, 1], c=Y,cmap=plt.cm.Set1_r,s=150)
            # Set common labels
            ax1.set_xlabel(('Mean Frequency in %s Niche'%niche_plot))
            ax1.set_ylabel('Niche')
            ax2.set_xlabel('PC-1')
